...
- when code is pushed or merged to `main` branch, artifiact repo namespace will be `LabNow` in our example.
- when code is pushed to a branch name in the pattern of `dev/*` and a PR is created to merge this branch `dev/*` into `main`, the artifact will be pushed to the namespace `LabNow0dev` (the suffix `0dev` can be customized).
