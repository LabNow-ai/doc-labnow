...
AI/数据科学的瑞士军刀——LabNow提供了一站式、开箱即用、可自由定制的，基于容器的、开源AI/数据科学开发、分析工具。

LabNow将常见的、最新的开放数据科学环境和工具封装成为了容器镜像，你无需进行繁琐的环境配置即可快速开始AI/数据科学的工程，同时能够方便地复现与分享您的研究工作。您可以在LabNow中使用Jupyter Notebook/JupyterLab中运行Python, R, OpenJDK, NodeJS, Go, Julia, Rust等语言，LabNow也封装了VS Code, R-Studio等工具。使用LabNow，您可以：

- 📦 避免繁琐的环境配置、安装过程，LabNow已经把常用的、最新的环境和工具封装在容器镜像中，您可以开箱即用；
- 🌍 让您的工作更容易被自己/他人`复现`——LabNow让科学研究和数据分析项目成为[可复现的工作流(reproducible pipelines)](https://doi.org/10.1038/d41586-018-07196-1)，这能让你更好地和同行[分享你的工作](https://doi.org/10.1038/515151a).
- 🆙 能让你的算法和关键创新更容易横向、纵向扩展，LabNow让你把自己在开发环境所作的工作无缝、极其简单地部署到生产环境，如提供RESTful API、进行map/reduce操作等。
...
LabNow封装、整理、维护了一些列的容器镜像，这些镜像包含了常见的开放AI/数据科学语言环境和安装包：Ptyhon, R, OpenJDK, NodeJS, Go, Julia, Rust等，同时封装了Jupyter Notebook / JupyterLab / VS Code / RStudio等IDE让用户便捷地进行开发、交互计算。LabNow适用于下面的应用场景：
...
- 从 [LabNow镜像功能列表](tutorial-basics/labnow-stacks-1-foundation.md) 中，选择你需要的功能包，如果你的磁盘空间和网速满足要求则推荐CPU用户选择`full`，GPU用户选择`full-cuda`。
...
例如：`REGISTRY="registry.cn-beijing.aliyuncs.com/LabNow/base-dev:full"`

#### Linux/macOS用户，在bash或终端中运行

```shell
WORKDIR="/root"             # <- macOS用户建议改为用户目录，如：/Users/your_user_name
IMG="LabNow/base-dev:latest"  # <- 你选择的功能包(Stack)
...
- 👉 使用`IMG="LabNow/LabNow:full-cuda"`或其他支持cuda的镜像
...
```cmd
SET IMG="LabNow/developer:latest"
SET WORKDIR="D:/work"
...
以上指令会下载docker镜像，建立一个名为LabNow的docker容器，并打印出一条含有48位十六进制密钥的URL地址。
